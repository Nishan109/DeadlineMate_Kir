      ...
      <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2 sm:space-x-4 min-w-0">
              ...
              <div className="flex items-center space-x-3">
                <StickyNote className="w-6 h-6 sm:w-8 sm:h-8 text-emerald-600" />
                <div className="min-w-0">
                  <h1 className="text-lg sm:text-2xl font-bold text-gray-900 truncate">Notes</h1>
                  <p className="text-xs sm:text-base text-gray-600 hidden sm:block">Organize your thoughts and ideas</p>
                </div>
              </div>
            </div>
        ...
        <main className="flex-1 p-3 sm:p-6 overflow-auto">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-3 sm:gap-4">
            <h2 className="text-base sm:text-xl font-semibold text-gray-900">Your Notes</h2>
            <Button
          ...
          <Tabs value={activeFilter} onValueChange={setActiveFilter} className="mb-4 sm:mb-6">
            <TabsList className="grid w-full grid-cols-2 lg:grid-cols-4 lg:w-auto h-auto">
              ...
              <TabsTrigger value="pinned" className="text-xs sm:text-sm py-2 px-2 sm:px-3">
                Pinned ({stats.pinned})
              </TabsTrigger>
              <TabsTrigger value="linked" className="text-xs sm:text-sm py-2 px-2 sm:px-3">
                Linked ({stats.linked})
              </TabsTrigger>
              <TabsTrigger value="solo" className="text-xs sm:text-sm py-2 px-2 sm:px-3">
                Solo ({stats.solo})
              </TabsTrigger>
            </TabsList>
          </Tabs>
            ...
            {filteredNotes.length === 0 ? (
              ...
              filteredNotes.map((note) => {
                ...
                return (
                  <Card
                    key={note.id}
                    className={`${getColorClasses(note.color)} transition-all duration-200 cursor-pointer transform hover:scale-[1.02] hover:shadow-lg`}
                    onClick={() => handleNoteClick(note)}
                  >
                    <CardHeader className="pb-3">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-start space-x-2 flex-1 min-w-0">
                          {note.is_pinned && <Pin className="w-4 h-4 text-gray-600 flex-shrink-0 mt-0.5" />}
                          <CardTitle className="text-sm font-semibold leading-tight truncate">{note.title}</CardTitle>
                        </div>
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-6 w-6 p-0 flex-shrink-0 opacity-0 group-hover:opacity-100 hover:opacity-100 transition-opacity"
                            >
                    ...
                    <CardContent className="pt-0 group">
                      ...
                      {linkedDeadline && (
                        <div className="flex items-center text-xs text-blue-600 mb-3">
                          ...
                          <span className="truncate">{linkedDeadline.title}</span>
                        </div>
                      )}
